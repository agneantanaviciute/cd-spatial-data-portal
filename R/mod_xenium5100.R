      ...
      column(
        ...
        conditionalPanel(
          ...
          selectizeInput(
            ns("gene"), "Gene",
            choices = NULL, multiple = FALSE,
            options = list(placeholder = "Search gene…", create = FALSE, maxOptions = 6000)
          )
        ),
        tags$hr(),
        tags$h6("Maximum 20k cells per sample will be plotted."),
...
mod_xenium5100_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      set_status("Fetching 5100-plex images…")
      imgs <- try(sb_distinct_images(), silent = TRUE)
    ...
    observe({
      set_status("Fetching 5100-plex genes…")
      genes <- try(sb_distinct_genes(), silent = TRUE)
      ...
      updateSelectizeInput(session, "gene",
                           ...
                           options  = list(placeholder = "Search gene…", maxOptions = 20000)
      )
    ...
    observeEvent(input$image, {
      ...
      withProgress(message = sprintf("Loading cells for '%s'…", input$image), value = 0, {
        set_status("Fetching 5100-plex cells…")
        dat <- try(sb_fetch_cells_full(input$image), silent = TRUE)
    ...
    observeEvent(list(input$mode, input$image, input$gene), {
      ...
      if (identical(input$mode, "Gene")) {
        ...
        withProgress(message = sprintf("Loading %s expression…", input$gene), value = 0, {
          ...
          ex <- try(sb_fetch_expression(input$image, input$gene), silent = TRUE)
    ...
    output$umap_plot <- renderPlot({
      ...
      if (identical(input$mode, "Gene")) {
        ...
        ggplot2::ggplot(dat, ggplot2::aes(umap_1, umap_2, colour = value)) +
          ...
          ggplot2::labs(colour = input$gene, title = "UMAP (Xenium 5100-plex)")
      } else {
        ggplot2::ggplot(dat, ggplot2::aes(umap_1, umap_2, colour = cellannotationbroad)) +
          ggplot2::geom_point(size = 0.6, alpha = 0.85) +
          ggplot2::coord_equal() + ggplot2::theme_minimal(base_size = 14) +
          ggplot2::labs(colour = "Cell type", title = "UMAP (Xenium 5100-plex)")
      }
    })
    ...
    output$spatial_plot <- renderPlot({
      ...
      if (identical(input$mode, "Gene")) {
        ...
        ggplot2::ggplot(dat, ggplot2::aes(x, y, colour = value)) +
          ...
          ggplot2::labs(colour = input$gene, title = "Spatial (Xenium 5100-plex)")
      } else {
        ggplot2::ggplot(dat, ggplot2::aes(x, y, colour = cellannotationbroad)) +
          ggplot2::geom_point(size = 0.6, alpha = 0.85) +
          ggplot2::coord_equal() + ggplot2::scale_y_reverse() + ggplot2::theme_minimal(base_size = 14) +
          ggplot2::labs(colour = "Cell type", title = "Spatial (Xenium 5100-plex)")
      }
    })
  })
}
